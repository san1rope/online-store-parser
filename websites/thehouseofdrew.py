...
async def parser_thehouseofdrew(session: aiocfscrape.CloudflareScraper, item_url: str, proxylist: list,
    ...
    try:
        ...
        image_link_html = soup_item.find(class_="product__media media media--transparent").find("img")["src"].split()
        image_link = ""
        for el in image_link_html:
            if "cdn.shopify.com/s/files" in el:
                image_link = "https:" + el
                break
        ...
        item_values = []
        try:
            select_html = soup_item.find(
                "noscript", class_=re.compile("product-form__noscript-wrapper-template--")).find_all("option")
        except AttributeError:
            size_name = "ONE_SIZE"
            full_size_price = soup_item.find("span", class_="price-item price-item--regular").text.strip()
            size_price = ""
...
async def thehouseofdrew(proxylist: list, proxy_all_values: list):
    main_url, file_name = "https://thehouseofdrew.com", "thehouseofdrew_-_6_3"
    request_delay = 0.1
    headers = {"user-agent": user_agent.generate_user_agent()}
        ...
        for url in sitemap_urls:
            ...
            for loc in all_tags:
                ...
                task = asyncio.create_task(
                    parser_thehouseofdrew(session, loc.text, proxylist, multiplier, request_delay))
                tasks.append(task)

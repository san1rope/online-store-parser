...
async def parser_stussy_uk(session: aiocfscrape.CloudflareScraper, item_url: str, proxylist: list, multiplier: float,
    ...
    try:
        ...
        if retry:
            return await parser_stussy_uk(session, item_url, proxylist, multiplier, request_delay, (retry - 1))


async def stussy_uk(proxylist: list, proxy_all_values: list):
    main_url, file_name = "https://uk.stussy.com", "stussy_uk_-_1_3"
    request_delay = 0.1
    headers = {"user-agent": user_agent.generate_user_agent()}

    agcm = gspread_asyncio.AsyncioGspreadClientManager(get_creds)

    primary_id = proxy_all_values[1][4]
    multiplier = float(proxy_all_values[1][7].replace(',', '.'))
    worksheet_id = proxy_all_values[1][3]
        ...
        for url in sitemap_urls:
            ...
            for loc in all_tags:
                ...
                task = asyncio.create_task(
                    parser_stussy_uk(session, loc.text, proxylist, multiplier, request_delay))
                tasks.append(task)

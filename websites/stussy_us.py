...
async def parser_stussy_us(session: aiocfscrape.CloudflareScraper, item_url: str, proxylist: list, multiplier: float,
                           request_delay: float = None, retry: int = 3):
    headers = {"user-agent": user_agent.generate_user_agent()}
    proxies = await get_proxy(proxylist)

    item_markup = await get_data(session=session, url=item_url, request_delay=request_delay, headers=headers,
                                 proxies=proxies)
    ...
    try:
        ...
        if retry:
            return await parser_stussy_us(session, item_url, proxylist, multiplier, request_delay, (retry - 1))


async def stussy_us(proxylist: list, proxy_all_values: list):
    main_url, file_name = "https://www.stussy.com", "stussy_us"
    request_delay = 0.1
    headers = {"user-agent": user_agent.generate_user_agent()}

    agcm = gspread_asyncio.AsyncioGspreadClientManager(get_creds)

    primary_id = proxy_all_values[0][4]
    multiplier = float(proxy_all_values[4][7].replace(',', '.'))
    worksheet_id = proxy_all_values[0][3]
        ...
        for url in sitemap_urls:
            ...
            for loc in all_tags:
                ...
                task = asyncio.create_task(
                    parser_stussy_us(session, loc.text, proxylist, multiplier, request_delay))
                tasks.append(task)
